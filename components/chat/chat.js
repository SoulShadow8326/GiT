				...
				btn.addEventListener('click', function(){
					...
					if (isAI) {
						...
						if (typeof Notyf !== 'undefined') {
							const n = new Notyf({ duration: 10000, types: [{ type: 'aidisclaimer', background: '#De3163', icon: false }] });
					...
					if (b) {
						...
						b.onclick = function(){
							...
							if (isAI) {
								...
								if (inputEl) inputEl.focus();
								if (typeof Notyf !== 'undefined') {
									const n = new Notyf({ duration: 200, types: [{ type: 'aidisclaimer', background: '#De3163', icon: false }] });
...
function updateChatIcon(hasNewMessages) {
    ...
    if (hasNewMessages) {
		...
		fill="none" stroke="#De3163" stroke-width="2"
